...
def model_fn(features, labels, mode, params):
    ...
    labels1 = tf.one_hot(labels.map(lambda x: x[0]), depth=vocab_size)
    labels2 = tf.one_hot(labels.map(lambda x: x[1]), depth=vocab_size)
    labels3 = tf.one_hot(labels.map(lambda x: x[2]), depth=vocab_size)

    loss = (
        tf.losses.softmax_cross_entropy(labels1, logits1)
        + tf.losses.softmax_cross_entropy(labels2, logits2)
        + tf.losses.softmax_cross_entropy(labels3, logits3)
    )
